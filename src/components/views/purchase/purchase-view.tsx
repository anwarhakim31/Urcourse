...
import PurchaseViewSkeleton from "./purchase-view-skeleton";

const PurchaseView = ({ purchaseId }: { purchaseId: string }) => {
  ...
  const { data, isLoading } = useGetPurchase(purchaseId);
  ...
  const handleContinue = () => {
    ...
    mutate({
      purchaseId: data.data?.id as string,
        ...
        <main className="pt-24 pb-16 bg-back">
          <div className="flex flex-col  lg:flex-row container gap-6">
            <div className="block lg:hidden  bg-white rounded-tr-md rounded-tl-md border p-6 w-full lg:w-[400px] h-fit">
              <DetailOrder purchase={data?.data} />
            </div>
            <div
                ...
                <div className="space-y-4 mt-4">
                  <p className="text-slate-900 text-sm flex justify-between items-center">
                    ...
                    <span className="font-semibold">
                      {formatCurrency(data?.data?.price as number)}
                    </span>
                  </p>
                  <p className="text-slate-900 text-sm flex justify-between items-center">
                    Tax Transaction{" "}
